...
func (h *KategoriHandler) GetAll(c *fiber.Ctx) error {
	...
	if err != nil {
		return response.Error(c, http.StatusInternalServerError, "Gagal mengambil data kategori", nil)
	}
...
func (h *KategoriHandler) Create(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return response.Error(c, http.StatusInternalServerError, "Gagal menambahkan kategori", nil)
	}
...
func (h *KategoriHandler) Update(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return response.Error(c, http.StatusInternalServerError, "Gagal memperbarui kategori", nil)
	}
...
func (h *KategoriHandler) Delete(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		if errors.Is(err, service.ErrKategoriHasProduk) {
			return response.Error(c, http.StatusConflict, err.Error(), nil)
		}
		return response.Error(c, http.StatusInternalServerError, "Gagal menghapus kategori", nil)
	}
